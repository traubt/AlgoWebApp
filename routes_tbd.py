...
def background_thread():
    ...
    while True:
        ...
        socketio.emit('my_response',
                      {'data': 'Bitcoin current price (USD): ' + price, 'count': count}, namespace='/algotrading')
...
@app.route('/binance_pair_history')
def binance_pair_history():
    headers = {}
    # if request.method == "GET":
    query = request.args.get('q')
    # print("tomer was here")
    candlesticks = client.get_historical_klines(query, Client.KLINE_INTERVAL_15MINUTE, "1 day ago UTC")
    candlestick = []
...
def crypto_bot():
    ...
    runalgo = bot(client,tf)
    runalgo._run_algo()
    return "Success"

@app.route('/equity_bot')
def equity_bot():
    tf = request.args.get('timeFrame')
    market = request.args.get('market')
    # if market == 'crypto':
...
def my_event(message):
    ...
    emit('my_response',
         {'data': message['data'], 'count': session['receive_count']}, namespace='/algotrading')

# Receive the test request from client and send back a test response
@socketio.on('test_message')
def handle_message(data):
    print('received message: ' + str(data))
    emit('test_response', {'data': 'Test response sent'}, namespace='/algotrading')

# Broadcast a message to all clients
@socketio.on('broadcast_message')
def handle_broadcast(data):
    print('received: ' + str(data))
    emit('broadcast_response', {'data': 'Broadcast sent'}, broadcast=True, namespace='/algotrading')
...
def connect():
    ...
    emit('my_response', {'data': 'Connected', 'count': 0}, namespace='/algotrading')
